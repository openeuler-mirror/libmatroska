...
mkdir -p build
cd build 
%cmake $OLDPWD/.  -DCMAKE_SHARED_LINKER_FLAGS="$LDFLAGS -Wl,--as-needed -Wl,--no-undefined -Wl,-z,now"
%__make %{?_smp_mflags}

%install
DESTDIR=%{buildroot} %__make install -C build
...
%changelog
* Fri Apr  2 2022 Jingwiw <ixoote@gmail.com> - 1.6.3-1
- init from openSUSE
  author: openSUSE
  url: https://build.opensuse.org/package/view_file/openSUSE:Factory:RISCV/libmatroska
  changes: https://build.opensuse.org/package/view_file/openSUSE:Factory:RISCV/libmatroska/libmatroska.changes